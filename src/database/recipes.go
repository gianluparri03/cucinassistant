...
func (r Recipes) New(name string) (Recipe, error) {
	...
	if err != nil {
		if pqe, ok := err.(*pq.Error); ok && pqe.Code == "23505" {
			...
			slog.Error("while creating new recipe:", "err", err)
...
func (r Recipes) GetAll() ([]Recipe, error) {
	...
	if err != nil {
		slog.Error("while retrieving recipes:", "err", err)
...
func (r Recipes) GetOne(RID int) (Recipe, error) {
	...
	if err != nil {
		return recipe, handleNoRowsError(err, r.uid, ERR_RECIPE_NOT_FOUND, "retrieving recipe")
	}
...
func (r Recipes) Edit(RID int, updated Recipe) (Recipe, error) {
	...
	if err != nil {
		if pqe, ok := err.(*pq.Error); ok && pqe.Code == "23505" {
			...
			slog.Error("while editing new recipe:", "err", err)
			return Recipe{}, ERR_UNKNOWN
		}
	}
...
func (r Recipes) Delete(RID int) error {
	...
	if err != nil {
		slog.Error("while deleting recipe:", "err", err)
		return ERR_UNKNOWN
